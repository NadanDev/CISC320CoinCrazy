...
#include "Utils/open_page.cpp"

int main(int argc, char* argv[]) {      
    const std::string addrStr = "127.0.0.1";
    const std::string portStr = "8080";
    const std::string originStr = "./Client/";
    const std::string threadStr = "1";
    auto address = net::ip::make_address(addrStr);
    auto port = static_cast<unsigned short>(std::stoi(portStr));
    auto doc_root = originStr;
    auto const threads = std::max<int>(1, std::stoi(threadStr));
    ...
    open("http://" + addrStr + ":" + portStr, originStr);
